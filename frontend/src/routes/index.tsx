...
function HeroBriefCard() {
  return (
    <div className="cp-fade-up relative" style={{ animationDelay: "120ms" }}>
      ...
      />
      <article
        className="relative overflow-hidden rounded-2xl border border-white/10 bg-white shadow-[var(--shadow-3)]"
        style={{ color: "#0A0A12" }}
      >
        <header className="flex items-center justify-between border-b border-black/10 px-6 py-4">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.16em]" style={{ color: "#4A4A57" }}>Executive brief</p>
            <h3 className="mt-1 font-display text-base font-semibold" style={{ color: "#0A0A12" }}>Whitfield v. Marrow Holdings</h3>
          </div>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-mint-soft px-2.5 py-1 font-mono text-[10px] font-medium uppercase tracking-wider" style={{ color: "#0A0A12" }}>
            <span className="h-1.5 w-1.5 rounded-full bg-mint cp-pulse" /> Handoff active
          </span>
        </header>
        <div className="space-y-4 px-6 py-5 text-sm leading-relaxed" style={{ color: "#0A0A12" }}>
          <p>
        ...
        <footer className="flex items-center justify-between border-t border-black/10 bg-black/[0.03] px-6 py-3 text-xs" style={{ color: "#4A4A57" }}>
          <span className="font-mono">12 sources · 184 pages indexed</span>
          <span className="inline-flex items-center gap-1" style={{ color: "#0A0A12" }}><Sparkles className="h-3 w-3 text-indigo" /> Generated by CasePass</span>
        </footer>
      </article>
    </div>
  );
...
function CiteChip({ doc, page }: { doc: string; page: number }) {
  return (
    <span
      className="inline-flex items-center gap-1 rounded border border-mint/60 bg-mint-soft/60 px-1.5 py-px align-middle font-mono text-[10px] font-medium"
      style={{ color: "#0A0A12" }}
    >
      {doc}, p.{page}
    </span>
...
function Mini({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-black/10 bg-black/[0.02] px-3 py-2">
      <p className="font-mono text-[10px] uppercase tracking-wider" style={{ color: "#4A4A57" }}>{label}</p>
      <p className="mt-1 font-display text-sm font-semibold" style={{ color: "#0A0A12" }}>{value}</p>
    </div>
  );
}

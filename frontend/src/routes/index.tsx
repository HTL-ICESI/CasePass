  ...
  return (
    <div className="cp-fade-up relative" style={{ animationDelay: "120ms" }}>
      <div
        ...
        aria-hidden
      />
      <article className="relative overflow-hidden rounded-2xl border border-white/10 bg-white text-foreground shadow-[var(--shadow-3)]">
        <header className="flex items-center justify-between border-b border-border/70 px-6 py-4">
          <div>
            <p className="font-mono text-[11px] uppercase tracking-[0.16em] text-muted-foreground">Executive brief</p>
            <h3 className="mt-1 font-display text-base font-semibold">Whitfield v. Marrow Holdings</h3>
          </div>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-mint-soft px-2.5 py-1 font-mono text-[10px] font-medium uppercase tracking-wider text-foreground">
        ...
        <footer className="flex items-center justify-between border-t border-border/70 bg-muted/40 px-6 py-3 text-xs text-muted-foreground">
          <span className="font-mono">12 sources · 184 pages indexed</span>
          <span className="inline-flex items-center gap-1 text-foreground"><Sparkles className="h-3 w-3 text-indigo" /> Generated by CasePass</span>
        </footer>
      </article>
    </div>
  );
...
function CiteChip({ doc, page }: { doc: string; page: number }) {
  return (
    <span className="inline-flex items-center gap-1 rounded border border-mint/60 bg-mint-soft/60 px-1.5 py-px align-middle font-mono text-[10px] font-medium text-foreground">
      {doc}, p.{page}
    </span>
  );
}
